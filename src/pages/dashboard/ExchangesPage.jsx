...
function ExchangeStatusBadge({ status }) {
  const cls   = status === 'pending' ? 'in-progress' : status === 'accepted' ? 'complete' : status === 'rejected' ? 'rejected' : 'expired'
  const label = status === 'pending' ? 'Pending' : status === 'accepted' ? 'Accepted' : status === 'rejected' ? 'Declined' : 'Expired'
...
export default function ExchangesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      authFetch('/exchanges/', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
      ...
      .catch(() => { setError('Failed to load exchanges'); setLoading(false) })
  }, [])
  ...
  async function handleAccept() {
    ...
    try {
      ...
      if (!res.ok) { setAcceptError(data.detail || 'Failed to accept'); return }
      setAcceptModal(null)
      setAcceptAppId('')
      reload()
    } catch {
  ...
  function isReviewDone(ex, prefix) {
    return ex[`${prefix}is_complete`] || ex[`${prefix}is_rejected`] || ex[`${prefix}is_expired`]
  }

  function isExchangeComplete(ex) {
    return ex.status === 'accepted' && isReviewDone(ex, 'ror_') && isReviewDone(ex, 'rod_')
  }

  const complete = exchanges.filter(isExchangeComplete)
  const active   = exchanges.filter(e => e.status === 'accepted' && !isExchangeComplete(e))
  const sentReqs = exchanges.filter(e => e.requester_id === currentUser.id && e.status !== 'accepted')
  const recvReqs = exchanges.filter(e => e.requestee_id === currentUser.id && e.status !== 'accepted')

  const list = tab === 'active' ? active : tab === 'sent' ? sentReqs : tab === 'received' ? recvReqs : complete
          ...
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <div>
                ...
                <div className="modal-url">{acceptModal.requester_username} will review <strong>{acceptModal.requester_app_name}</strong></div>
              </div>
            </div>
            {acceptModal.message && (
            ...
            <p className="modal-section-label" style={{ padding: '0 24px' }}>WHICH OF YOUR APPS WILL YOU REVIEW IN RETURN?</p>
            ...
            <div className="modal-actions">
              <button className="modal-btn-cancel" onClick={() => { setAcceptModal(null); setAcceptAppId('') }}>Decline</button>
              <button className="modal-btn-start" onClick={handleAccept} disabled={accepting || !acceptAppId}>
      ...
      <div className="exchanges-body">
        <div className="reviews-tabs">
          {TABS.map(({ key, label, count }) => (
            <button
              ...
              onClick={() => setTab(key)}
            >
              {label}{count > 0 && <span className="exchange-tab-count">{count}</span>}
            </button>
          ))}
        </div>
        ...
        {!loading && !error && list.length > 0 && (
          <div className="exchanges-list">
            {list.map(ex => {
              const timeLeft = ex.status === 'pending' ? formatTimeRemaining(ex.expires_at) : null
              const isUrgent = timeLeft && (timeLeft.startsWith('0') || (!timeLeft.includes('d') && !timeLeft.includes('h')))
              const isPendingReceived = ex.status === 'pending' && ex.requestee_id === currentUser.id

              // review_of_requester = B reviews A's app (requester_app)
              // review_of_requestee = A reviews B's app (requestee_app)
              const rorStatus = ex.status === 'accepted' ? {
                is_submitted: ex.ror_is_submitted, is_complete: ex.ror_is_complete,
                is_rejected: ex.ror_is_rejected, is_expired: ex.ror_is_expired,
                review_requested: ex.ror_review_requested,
              } : null
              const rodStatus = ex.status === 'accepted' ? {
                is_submitted: ex.rod_is_submitted, is_complete: ex.rod_is_complete,
                is_rejected: ex.rod_is_rejected, is_expired: ex.rod_is_expired,
                ...
                <div key={ex.id} className="exchange-card">
                  <div className="exchange-card-apps">
                    <div className="exchange-app-pill">
                      ...
                      <div>
                        <div className="exchange-app-name">{ex.requester_app_name}</div>
                        <div className="exchange-app-by">by {ex.requester_username}</div>
                      </div>
                    </div>
                    ...
                    <div className="exchange-app-pill">
                      {ex.requestee_app_id ? (
                        <>
                          <div className="exchange-app-icon" style={{ background: ex.requestee_app_color }}>{ex.requestee_app_initials}</div>
                          <div>
                            <div className="exchange-app-name">{ex.requestee_app_name}</div>
                            <div className="exchange-app-by">by {ex.requestee_username}</div>
                          </div>
                        </>
                      ) : (
                        <div className="exchange-app-tbd">
                          <div className="exchange-app-icon exchange-app-icon--tbd">?</div>
                          <div>
                            <div className="exchange-app-name">{ex.requestee_username}'s app</div>
                            <div className="exchange-app-by">pending acceptance</div>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="exchange-card-footer">
                    <div className="exchange-card-status">
                      {(tab === 'sent' || tab === 'received') && <ExchangeStatusBadge status={ex.status} />}
                      {timeLeft && (
                    ...
                    <div className="exchange-card-actions">
                      {isPendingReceived && (
                        <>
                          ...
                          <button className="exchange-accept-btn" onClick={() => { setAcceptModal(ex); setAcceptAppId('') }}>Accept</button>
                        </>
                      )}
                      {ex.status === 'accepted' && (
                        <div className="exchange-review-links">
                          {ex.review_of_requester && (
                            <div className="exchange-review-link-row">
                              ...
                              <button className="exchange-review-btn" onClick={() => navigate(
